...
describe("deleteMessage action", () => {
	it("should have correct name and description", () => {
		...
		expect(deleteMessage.description).toBe(
	...
	it("should validate only for discord source", async () => {
		...
		expect(await deleteMessage.validate(mockRuntime, discordMessage)).toBe(
			true,
		);
		expect(await deleteMessage.validate(mockRuntime, telegramMessage)).toBe(
			false,
		);
	});
