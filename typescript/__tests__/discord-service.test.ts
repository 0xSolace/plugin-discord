...
describe("Discord Service Types", () => {
	...
	describe("DiscordEventTypes", () => {
		it("should have MESSAGE_RECEIVED event", () => {
			expect(DiscordEventTypes.MESSAGE_RECEIVED).toBe(
		...
		it("should have REACTION_RECEIVED event", () => {
			expect(DiscordEventTypes.REACTION_RECEIVED).toBe(
		...
		it("should have VOICE_STATE_CHANGED event", () => {
			expect(DiscordEventTypes.VOICE_STATE_CHANGED).toBe(
		...
		it("should have permission audit events", () => {
			...
			expect(DiscordEventTypes.ROLE_PERMISSIONS_CHANGED).toBe(
				"DISCORD_ROLE_PERMISSIONS_CHANGED",
			);
			expect(DiscordEventTypes.MEMBER_ROLES_CHANGED).toBe(
	...
	describe("validateSnowflake", () => {
		...
		it("should throw for too short snowflake", () => {
			expect(() => validateSnowflake("1234567890123456")).toThrow(
				DiscordPluginError,
			);
		});

		it("should throw for too long snowflake", () => {
			expect(() => validateSnowflake("12345678901234567890")).toThrow(
				DiscordPluginError,
			);
		});

		it("should throw for snowflake with letters", () => {
			expect(() => validateSnowflake("1234567890123456a")).toThrow(
				DiscordPluginError,
			);
		});
	...
	describe("Error classes", () => {
		...
		it("should create DiscordConfigurationError", () => {
			...
			expect(error.message).toBe(
				"Missing required configuration: DISCORD_API_TOKEN",
			);
